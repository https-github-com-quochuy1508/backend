...
const { /* sequelize, Op, */ groupUsers, places } = models;
...
export default class PreCheckHelpers {
        ...
        if (condition) {
          try {
            ...
            modelCallback(optionsCallback)
              ...
              .catch((error) => {
                reject(
                  PreCheckHelpers.createErrorCheck(
                    { typeCheck: TYPE_CHECK.GET_INFO, modelStructure },
                    error
                  )
                );
              });
  ...
  static createErrorCheckNew(infoCheck, error) {
    ...
    return ErrorHelpers.errorReject(
      error,
      detail.type,
      "",
      202,
  ...
  static createMidMessWithOptions(arr, options) {
    return arr.reduce((mess, e, i) => {
      if (!Array.isArray(e)) {
        return i === 0
          ? viMessage[e]
          : mess + ` ${options.shift()} ` + viMessage["api." + e];
      } else
        return i === 0
  ...
  static createMidMess(arr) {
    return arr.reduce((mess, e, i) => {
      if (!Array.isArray(e)) {
        ...
        return i === 0
          ? PreCheckHelpers.createMidMess(e)
          : mess + ` chứa ` + PreCheckHelpers.createMidMess(e);
    }, "");
  }
    ...
    if (userInfo) {
      switch (Number(userInfo.groupUserId)) {
        ...
        case GROUP_ADMINSTRATOR_CLINIC:
          ...
          return (
            Array.from(placesIds).map(Number).indexOf(Number(placesId)) !== -1
          );
  ...
  static async getInfoUser(auth) {
    ...
    const infoUser = await userModel.findOne({
      ...
      include: [
        { model: groupUsers, as: "groupUser", attributes: ["id"] },
        {
          model: places,
          as: "places",
          attributes: [
   ...
   */
  static main() {
    const midMes = PreCheckHelpers.createMidMess(
      ["adsType.name", ["adsType.name", "adsType.name"]],
      ["chứa", "và"]
    );
