...
export default {
  get_list: (req, res, next) => {
    ...
    try {
      ...
      // const usersId = (req.auth && req.auth.userId) || undefined;
      ...
      userCourseService
        ...
        .then((data) => {
          ...
          res.header(
            "Content-Range",
            `sclSocialAccounts ${range}/${data.count}`
          );
          res.send(dataOutput);
          // write log
  ...
  get_one: (req, res, next) => {
    ...
    try {
      ...
      // console.log("userCourseService param: ", param)
      userCourseService
        .get_one(param)
        .then((data) => {
          res.send(data);
  ...
  updateByUserId: (req, res, next) => {
    ...
    try {
      ...
      // const entity = req.body
      const param = { usersId, entity };

      userCourseService
        .updateByUserId(param)
        .then((data) => {
          if (data && data.result) {
            const dataOutput = {
              result: data.result,
